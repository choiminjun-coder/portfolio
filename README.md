...
# 🎮 Unity Developer Portfolio – 최민준

---

## 👋 소개

안녕하세요! 유니티 기반 3D 게임 개발자 **최민준**입니다.  
기획 의도 분석과 구조적인 시스템 구현에 강점을 가지고 있으며,  
사용자 경험과 퍼포먼스 최적화를 고려한 개발을 지향합니다.

- 유니티 개발 시작: **2021년 9월 (4년차)**
- 전문 분야: Unity 기반 TPS/FPS 컨트롤러, UI 시스템, 게임 전반 구조 설계
- 협업 경험: 팀 프로젝트 다수 진행 (외부 개발자/디자이너와 협업)
...
- 담당 역할:  
  ...
  - 컬러 카운터 시스템  
  - 엔딩 연출
- 주요 기술 요소: FSM 기반 캐릭터 이동, 컬러 기반 스테이지 클리어, 폭발 연출
...
⏱ 개발 기간: 2024.09.01 ~ 2024.09.30

### 🎮 프로젝트 개요

- 장르: 1인칭 FPS + 서바이벌  
- 참여 인원: 1인 (기획 ~ 구현 전담)  
- 콘텐츠: 무한 적 처치, 생존 기반 점수, 랜덤 아이템 강화
...
#### ▶ 총알 처리 로직 (`Bullet.cs`)
- `Start()` – 플레이어의 `CharacterStats`로부터 공격력 연동  
...
- 효과는 `Coroutine`을 통해 일정 시간 후 자동 복구  
